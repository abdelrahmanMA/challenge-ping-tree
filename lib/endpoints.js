...
function addNewTarget (req, res, opts, cb) {
  body(req, res, function (err, data) {
    if (err) return _badRequest(req, res, err)
...
function updateTargetById (req, res, opts, cb) {
  _validateIdParam(opts.params, function (err, id) {
    ...
    body(req, res, function (err, data) {
      if (err) return _badRequest(req, res, err)
      _validateUpdateTarget(data, function (err, data) {
        if (err) return _badRequest(req, res, err)
...
function _validateTarget (data, cb) {
  ...
  if (
    !Array.isArray(data.accept && data.accept.geoState && data.accept.geoState.$in) ||
    !Array.isArray(data.accept && data.accept.hour && data.accept.hour.$in)
  ) {
...
function _validateUpdateTarget (data, cb) {
  if (
    ![
      data.id,
      data.url,
    ...
    return cb(new Error('At least one field is required'), null)
  }
  if (
    !Array.isArray((data.accept && data.accept.geoState && data.accept.geoState.$in)) ||
    !Array.isArray((data.accept && data.accept.hour && data.accept.hour.$in))
...
function _badRequest (req, res, err) {
  return send(req, res, {
    statusCode: 400,
    body: { status: err.message || 'Something went wrong' }
  })
}
